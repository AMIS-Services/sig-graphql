...
Exception handling is an important part of every application. Spring is equipped with a `DefaultGraphQLErrorHandler` and you might have seen some errors generated by this handler; they are of the form:
```javascript
"errors": [
    {
        "message": "Validation error of type SubSelectionRequired: Sub selection required for type null of field people",
        "description": "Sub selection required for type null of field people",
        "errorType": "ValidationError"
    }
]
```
Notice that the `"errors"` key holds an array; the default error handler will collect all errors in your request, bundle them and return them. Each error has an `"errorType"`, a `"message"` and a `"description"` and may contain other properties that are specific to certain errors - for instance the `"ValidationError"`.

Change the `createPerson` mutation to throw an exception if the name in the request already exists (hint: get an Optional<Person> from the personRepository by name and throw a `RuntimeException` if it exists). What exception do you get when you try creating a Person with a name that already exists?

Unfortunately the `DefaultGraphQLErrorHandler` filters all unknown errors and simply returns "Internal Server Error(s) while executing query". That's not desirable!

You can override the `DefaultGraphQLErrorHandler` as a configuration:
```java
@Configuration
...
```
This is a simplified version of the `DefaultGraphQLErrorHandler` and shows that exceptions are filtered and if exceptions are filtered a `GenericGraphQLError` is added. Change this implementation to also show `RuntimeException`s - of if you created a custom exception, you can show that exception.